...
program = statement
repeat_until_statement = "repeat" block_statement "until" expression;
...
{ "tag":"while", "condition":<expression_node>, 
    "do":<statement_node>}
{"tag":"repeat_until", "block": <block_statement_node>, "condition":<expression_node>
    "do:<statement_node>}
...
def parse_repeat_until(tokens):
    """
    repeat_until_statement = "repeat" block_statement "until" expression;
    """
    assert tokens[0]["tag"] == "repeat"
    tokens = tokens[1:]  
    block, tokens = parse_block_statement(tokens)  

    assert tokens[0]["tag"] == "until"
    tokens = tokens[1:]  
    condition, tokens = parse_expression(tokens)  

    return {"tag": "repeat_until", "block": block, "condition": condition}, tokens


def test_parse_repeat_until():
    """
    repeat_until_statement = "repeat" block_statement "until" expression;
    """
    ast = parse_repeat_until(t("repeat { x = x+1; } until (x >= 10)"))[0]
    assert ast == {
        "tag": "repeat_until",
        "block": {
            "tag": "block",
...
def parse_statement(tokens):
    ...
    if tokens[0]["tag"] == "repeat":
        return parse_repeat_until(tokens)
